...
!------------------------for real matver------------------------
  ...
  !SUBROUTINE ISO_rmatvec(veff,p,q,dimen)
     ...
     !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#ifndef MPI
     call memory_sum('ISO_rmatvec',real(size(thrq),DP)*3*DP+size(sphere_temp)*DP+n*DP)
#else
     call memory_sum('ISO_rmatvec',real(size(thrq),DP)*2*DP+size(Tsp)*DP+n*DP)
#endif
#ifndef MPI
     !transfrom vec to 3D
     CALL ISO_sphere2grid(p,thrq)
     !nonlocal part
     gridp=reshape(thrq,(/n/))
#endif
     IF(LDG)THEN
     ...
     do Iy=1,n2
     do Iz=1,n3
     Tsp(Ix,Iy,Iz)=Tsp(Ix,Iy,Iz)+thrq(Ix,Iy,Iz)*veff_3D(Ix,Iy,Iz) !reshape(thrq*veff_3D,(/n1,n2,n3/))
     enddo
     enddo
     enddo
     do I=1,dimen,1
        sphere_temp(I)=Tsp(rho_calc%x(I),rho_calc%y(I),rho_calc%z(I))
     enddo
     CALL DAXPY(dimen,1.d0,sphere_temp,1,q,1)
#else
...
! #endif
#ifndef MPI
     call memory_free('ISO_rmatvec',real(size(thrq),DP)*3*DP+size(sphere_temp)*DP+n*DP)
#else
