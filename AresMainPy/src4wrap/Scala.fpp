...
# 1 "Scala.f90"
    ...
    Subroutine Init_scala()  !{{{
      USE parameters, ONLY: Lpbc,Nstates_global
...
!
    ...
    !use system_info
    implicit none
    complex(dp),intent(in)  :: amat(:,:),bmat(:,:)
    ...
    Subroutine SL_matmat_gridcn(opa,opb,amat,bmat,cmat,am,an,bm,bn,amb,anb,bmb,bnb,cmb,cnb)  !{{{
      ...
      complex(dp)     :: alpha = 1.d0, & 
           & beta = 0.d0
      complex(dp),allocatable :: amat_y(:,:),bmat_y(:,:)&
    ...
    End Subroutine SL_matmat_gridcn  !}}}

    Subroutine SL_matmat_gridnn(opa,opb,amat,bmat,cmat,am,an,bm,bn,amb,anb,bmb,bnb,cmb,cnb)  !{{{
      USE smpi_math_module, ONLY: MPI_complex16,mpinfo,MPI_SUM
      implicit none
      complex(dp),intent(in)  :: amat(:,:),bmat(:,:)
      complex(dp),intent(out)  :: cmat(:,:)
      ...
      complex(dp)     :: alpha = 1.d0, & 
           & beta = 0.d0
      complex(dp),allocatable :: Bmat_local(:,:),Bmat_global(:,:),amat_y(:,:)
      integer(i4b) :: lda,ldb,ldc
    ...
    End Subroutine SL_matmat_gridnn  !}}}
...
!endif 
    ...
    call pzhegvx(1,'V','A','U',an,amat,1,1,desca,bmat,1,1,&
         &descb,0.d0,0.d0, 0,0,1e-7,m,nevec,eval,1e-9,evec,1,1&
    ...
    call pzhegvx(1,'V','A','U',an,amat,1,1,desca,bmat,1,1&
         &, descb,0.d0,0.d0,0,0,1e-7,m,nevec,eval,1e-9,&
    ...
    ENDIF
    !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ENDSUBROUTINE SL_GeneralizeEigen  !}}}
    SUBROUTINE SL_GeneralizeEigen_real2(dime,amat,bmat,am,an,bm,bn,amb,anb,bmb,bnb,evec,cm,cn,eval,cmbin,cnbin)  !{{{
    IMPLICIT NONE
    integer(i4b),intent(in) :: dime
    REAL(DP),INTENT(IN)  :: amat(:,:),bmat(:,:)
    ...
    allocate(iwork(3))
    allocate(rwork(3))! allocate(rwork(5000000))
    call pdsygvx(1,'V','A','U',an,amat,1,1,desca,bmat,1,1,&
         &descb,0.d0,0.d0, 0,0,1e-7,m,nevec,eval,1e-9,evec,1,1&
    ...
    call pdsygvx(1,'V','A','U',an,amat,1,1,desca,bmat,1,1&
         &, descb,0.d0,0.d0,0,0,1e-7,m,nevec,eval,1e-9,&
         &evec,1,1,descz,work,lwork,iwork,liwork&
         &,ifail,iclustr,gap,info )
    ! call pdsygvx(1,'V','A','U',an,amat,1,1,desca,bmat,1,1&
    !      &,descb,0.d0,0.d0,1,an,abtol,m,nevec,eval,0,&
    !      &evec,1,1,descz,work,lwork,iwork,liwork,ifail,iclustr,gap,info )
    ...
    ENDIF
    !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ENDSUBROUTINE SL_GeneralizeEigen_Real2  !}}}
...
!endif 
    ...
    abtol=2*pdlamch(blacs_contxt,'s')
...
!if (parallel%isroot) print * ,'debug lwork' , lwork ,'liwork',liwork,'iceil',iceil(neig,parallel%numprocs)
    call pdsygvx(1,'V','A','U',an,amat,1,1,desca,bmat,1,1,descb,0.d0,0.d0, &
                & 1,an,1e-7,m,nevec,eval,1e-9,evec,1,1,descz,work,lwork,    &
...
!if (parallel%isroot) print *,'the number of eigenvector',m
    ...
    ENDIF
    !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ENDSUBROUTINE SL_GeneralizeEigen_real  !}}}
    ...
    ENDSUBROUTINE twoD_map_set
    Subroutine SL_matmat_realtn(opa,opb,amat,bmat,cmat,am,an,bm,bn,amb,anb,bmb,bnb,cmb,cnb)  !{{{
      ...
      !> allreduce the local result
      CALL MPI_ALLREDUCE(Cmat_local, cmat_global, size(cmat_local), MPI_REAL8,&
           & MPI_SUM, parallel%comm, mpinfo)
      ! & MPI_SUM, parallel%commx, mpinfo)
    ...
    End Subroutine SL_matmat_realtn  !}}}

    Subroutine SL_matmat_realnn(opa,opb,amat,bmat,cmat,am,an,bm,bn,amb,anb,bmb,bnb,cmb,cnb)  !{{{
      USE smpi_math_module, ONLY: MPI_real8,mpinfo,MPI_SUM
      implicit none
      real(dp),intent(in)  :: amat(:,:),bmat(:,:)
      real(dp),intent(out)  :: cmat(:,:)
    ...
    End Subroutine SL_matmat_realnn  !}}}
