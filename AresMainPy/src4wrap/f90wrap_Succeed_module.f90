...
subroutine f90wrap_init_succeed_rho_real(n_rho, n_r, n_s, nspin, dv)
    use succeed, only: init_succeed_rho_real
    implicit none
    ...
    integer(4) :: nspin
    real(8) :: dv
    call init_succeed_rho_real(n_rho=n_rho, n_r=n_r, n_s=n_s, nspin=nspin, dv=dv)
end subroutine f90wrap_init_succeed_rho_real

subroutine f90wrap_init_succeed_rho_cmplx(n_rho, n_r, n_s, n_k, nspin, dv)
    use succeed, only: init_succeed_rho_cmplx
    implicit none
    ...
    integer(4) :: nspin
    real(8) :: dv
    call init_succeed_rho_cmplx(n_rho=n_rho, n_r=n_r, n_s=n_s, n_k=n_k, nspin=nspin, dv=dv)
end subroutine f90wrap_init_succeed_rho_cmplx

subroutine f90wrap_destroy_succeed
    use succeed, only: destroy_succeed
    implicit none
    
    call destroy_succeed()
end subroutine f90wrap_destroy_succeed

subroutine f90wrap_store_rho(n_rho, nspin, rho, dvol, n0, n1)
    ...
    call store_rho(n_rho=n_rho, Nspin=nspin, rho=rho, dvol=dvol)
...
subroutine f90wrap_get_rho(nr, r_new, nrho, nspin, rho_new, dvol, n0, n1, n2)
...
subroutine f90wrap_store_psi_cmplx(n_rho, n_s, n_k, nspin, psi, n0, n1, n2, n3)
    ...
    integer(4) :: n_k
    integer(4) :: nspin
    complex(8), dimension(n0,n1,n2,n3) :: psi
    integer :: n0
    !f2py intent(hide), depend(psi) :: n0 = shape(psi,0)
    integer :: n1
    !f2py intent(hide), depend(psi) :: n1 = shape(psi,1)
    ...
    call store_psi(n_rho=n_rho, n_s=n_s, n_k=n_k, nspin=nspin, psi=psi)
end subroutine f90wrap_store_psi_cmplx

subroutine f90wrap_store_psi_real(n_rho, n_s, nspin, psi, n0, n1, n2)
    use succeed, only: store_psi
    implicit none
...
end subroutine f90wrap_store_psi_real

subroutine f90wrap_get_psi_cmplx(nrho, n_s, n_k, nspin, psi_new, n0, n1, n2, n3)
    ...
    integer(4), intent(in) :: nspin
    complex(8), intent(inout), dimension(n0,n1,n2,n3) :: psi_new
    integer :: n0
    ...
    integer :: n3
    !f2py intent(hide), depend(psi_new) :: n3 = shape(psi_new,3)
    call get_psi(nrho=nrho, n_s=n_s, n_k=n_k, nspin=nspin, psi_new=psi_new)
end subroutine f90wrap_get_psi_cmplx

subroutine f90wrap_get_psi_real(nrho, n_s, nspin, psi_new, n0, n1, n2)
    use succeed, only: get_psi
    implicit none
...
end subroutine f90wrap_get_psi_real
...
subroutine f90wrap_succeed__array__psi1_c(dummy_this, nd, dtype, dshape, dloc)
    use constants
    use struct_module
    use smpi_math_module
    use succeed, only: succeed_psi1_c => psi1_c
    implicit none
    integer, intent(in) :: dummy_this(2)
    integer, intent(out) :: nd
    integer, intent(out) :: dtype
    ...
    nd = 4
    dtype = 15
    if (allocated(succeed_psi1_c)) then
        dshape(1:4) = shape(succeed_psi1_c)
        dloc = loc(succeed_psi1_c)
    else
        dloc = 0
    end if
end subroutine f90wrap_succeed__array__psi1_c

subroutine f90wrap_succeed__array__psi2_c(dummy_this, nd, dtype, dshape, dloc)
    use constants
    use struct_module
    use smpi_math_module
    use succeed, only: succeed_psi2_c => psi2_c
    implicit none
    integer, intent(in) :: dummy_this(2)
    integer, intent(out) :: nd
    integer, intent(out) :: dtype
    ...
    nd = 4
    dtype = 15
    if (allocated(succeed_psi2_c)) then
        dshape(1:4) = shape(succeed_psi2_c)
        dloc = loc(succeed_psi2_c)
    else
        dloc = 0
    end if
end subroutine f90wrap_succeed__array__psi2_c

subroutine f90wrap_succeed__array__psi3_c(dummy_this, nd, dtype, dshape, dloc)
    use constants
    use struct_module
    use smpi_math_module
    use succeed, only: succeed_psi3_c => psi3_c
    implicit none
    integer, intent(in) :: dummy_this(2)
    integer, intent(out) :: nd
    integer, intent(out) :: dtype
    ...
    nd = 4
    dtype = 15
    if (allocated(succeed_psi3_c)) then
        dshape(1:4) = shape(succeed_psi3_c)
        dloc = loc(succeed_psi3_c)
    else
        dloc = 0
    end if
end subroutine f90wrap_succeed__array__psi3_c
