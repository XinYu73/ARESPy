...
! #endif
...
#ifndef MPI
      ALLOCATE(rho_calc%x(rho_calc%OneDLength))
      ALLOCATE(rho_calc%y(rho_calc%OneDLength))
      ALLOCATE(rho_calc%z(rho_calc%OneDLength))
      ALLOCATE(rho_calc%OneDVeff(rho_calc%OneDLength))
      ALLOCATE(rho_calc%OneDSphere(rho_calc%OneDLength,Nspin))
      ALLOCATE(rho_calc%OneDwvf(rho_calc%OneDLength,Nstates,nk,NSPIN))
      ALLOCATE(rho_calc%OneDval(Nstates,nk,NSPIN))
      call memory_sum('rho_calc',real(size(rho_calc%x),DP)*I4B*3&
      ...
      if(.not.Llastrho)then
         CALL init_succeed_rho_real(rho_calc%OneDLength,natom,Nstates,Nspin,dvol)
      endif
#else
      ...
      !> assign the grid map
#ifndef MPI
      rho_calc%x=xyz(1,1:rho_calc%OneDLength)
      rho_calc%y=xyz(2,1:rho_calc%OneDLength)
      rho_calc%z=xyz(3,1:rho_calc%OneDLength)
#else
  ...
  SUBROUTINE confirm_iso_radius()
    ...
    IF(.not.Lpbc)THEN
       ...
       IF(Lradius_auto)THEN
          ...
          !> prompt the setting
#ifndef MPI
          print*,'PROMPT : the radius is reseted,',IsoRmax,'(ang) is properly'
#else
