...
contains
  ...
  !  Copyright (C) Stefan Goedecker, 1 Stuttgart, Germany, 1995, 1999
  !  Copyright (C) Stefan Goedecker, CEA Grenoble, 2002
  !  This file is distributed under the terms of the
  !  GNU General Public License, see http://www.gnu.org/copyleft/gpl.txt .
  ...
  subroutine fft(n1, n2, n3, nd1, nd2, nd3, z, isign, inzee)
    ...
    if (ncache == 0) then
      ...
      ALLOCATE(zw(1:2,1:ncache/4,1:2))
      ALLOCATE(trig(1:2,1:1024))
      ALLOCATE(after(1:20))
      ALLOCATE(now(1:20))
      ALLOCATE(before(1:20))
  ...
  !  Copyright (C) Stefan Goedecker, Cornell University, Ithaca, USA, 1994
  !  Copyright (C) Stefan Goedecker, 1 Stuttgart, Germany, 1999
  !  This file is distributed under the terms of the
  !  GNU General Public License, see http://www.gnu.org/copyleft/gpl.txt .
...
!     ALLOCATE(zw(1:2, 1:ncache/4, 1:2))
!     ALLOCATE(zt(1:2, 1:lzt, 1:n1))
!     ALLOCATE(zmpi2(1:2, 1:n1, 1:md2/nproc, 1:nd3))
!     ALLOCATE(cosinarr(1:2, 1:n3/2))

!     if (nproc > 1) then
!       ALLOCATE(zmpi1(1:2, 1:n1, 1:md2/nproc, 1:nd3/nproc, 1:nproc))
!     end if
...
!   !Allocations
!   ALLOCATE(trig1(1:2,1:8192))
!   ALLOCATE(after1(1:7))
!   ALLOCATE(now1(1:7))
!   ALLOCATE(before1(1:7))
!   ALLOCATE(trig2(1:2,1:8192))
!   ALLOCATE(after2(1:7))
!   ALLOCATE(now2(1:7))
!   ALLOCATE(before2(1:7))
!   ALLOCATE(trig3(1:2,1:8192))
!   ALLOCATE(after3(1:7))
!   ALLOCATE(now3(1:7))
!   ALLOCATE(before3(1:7))
!   ALLOCATE(zw(1:2,1:ncache/4,1:2))
!   ALLOCATE(zt(1:2,1:lzt,1:n1))
!   ALLOCATE(zmpi2(1:2,1:n1,1:nd2/nproc,1:nd3))
!   ALLOCATE(cosinarr(1:2,1:n3/2))
!   if (nproc > 1) then
!     ALLOCATE(zmpi1(1:2,1:n1,1:nd2/nproc,1:nd3/nproc,1:nproc))
!   end if
