...
CONTAINS
   ...
   SUBROUTINE GRayleigh_Ritz(Ik,veff,X,D)
      ...
      !
#ifndef MPI
      COMPLEX(DP),DIMENSION(sn,sn) :: S_hat,H_hat,Q
#else
...
#ifndef MPI
      CALL GeneralizeEigen(sn,H_hat,S_hat,Q,D)
#else
            ...
            DO Il=0,struct%Lmax(Ity)
               DO Im=-Il,Il
                  ...
                  DO Ip=1,n
                    ...
                    !init
#ifndef MPI
                    initX_sto(Ip,Ii)=initX_sto(Ip,Ii)+CMPLX(f,0.d0)
#else
...
! stop
   ...
   SUBROUTINE first_subspace_stopw(Ik,veff,Nmax,nev,initX,X,D)
      ...
      !LOCAL
#ifndef MPI
      COMPLEX(DP),DIMENSION(Nmax,Nmax) :: Shat,Hhat,Qs
#else
      COMPLEX(DP),DIMENSION(twoD_map(1,parallel%rankx,&
...
#ifndef MPI
      CALL matmat(initX,initX,'C','N',Shat)
#else
      mb=1 !n1*n2*n3
      nb=1!BLOCK_MBNB
      cmb=1!BLOCK_MBNB
      ...
      !eigen-decomposion
#ifndef MPI
      CALL GeneralizeEigen(Nmax,Hhat,Shat,Qs,D)
#else
...
#ifndef MPI
      USE Lapack_module , ONLY: OrthNorm_real
      USE Begin_module , ONLY: ISO_init_MO !, get_MO_coefficient
      USE Read_module , ONLY: get_MO_coefficient, destroy_MOinit
#endif
      USE m_time_evaluate, ONLY: memory_sum, memory_free
      IMPLICIT NONE
      !INOUT
...
#include "my_macro.h"
      ...
      open(111,file="veff_p")
      write(111,*)veff(1:1000,1)
      close(111)
      endif
#else
      CALL ISO_Vsphere(veff,Veff_sphere)
      open(111,file="veff_s")
      write(111,*)veff_sphere(1:1000,1)
      close(111)
#endif
#endif
      ...
      !allocate
#ifndef MPI
      ALLOCATE(initX(rho_calc%OneDLength,Nmax))
#else
      ...
      DO Is=1,Nspin
#ifndef MPI
         CALL ISO_first_subspace_sto_rand(Veff(:,:,:,Is), &
              &   Nmax,nev,initX,psi(:,:,Is),eval(:,Is))
#else
         Veff_local=Veff(:,Is)
         psi_local=psi(:,:,Is)
   ...
   ELSEIF(IDinit==1)THEN  !random initial orbital
      ...
      DO Is=1,Nspin
        ...
        ENDDO
#ifndef MPI
      ELSEIF(IDinit==2)THEN  !molecular orbital
        Nmax=nev
        Nrand=0
        !ALLOCATE(initX(rho_calc%OneDLength,Nmax))
        !initX=rho_calc.initMO
        !orth-Norm if need
        !CALL OrthNorm_real(rho_calc.initMO)
        !Raleigh-Ritz step
        !all spin
        ...
        DO Is=1,Nspin
          CALL ISO_first_subspace_sto_rand(Veff(:,:,:,Is), &
          &   Nmax,nev,rho_calc%initMO,psi(:,:,Is),eval(:,Is))
        ENDDO
#endif
      ENDIF
...
! #endif
   ...
   SUBROUTINE ISO_first_subspace_sto_rand(veff_3D,Nmax,nev,initX,X,D)
      ...
      !projected hamiltonian
#ifndef MPI
      CALL Rayleigh_quotient_ISO(veff_3D,Nmax,initX,Hhat)
#else
      CALL Rayleigh_quotient_ISO(veff_3D,nev,initX,Hhat_local)
#endif
      !eigen-decomposion
#ifndef MPI
      ! open(111,file="Hhat_s")
      !   do i=1,size(Hhat,2),1
      !      write(111,'(1000(F8.4,2X))')Hhat(:,i)
      !   enddo
      ! close(111)
      CALL GeneralizeEigen_real(Nmax,Hhat,Shat,Qs,Ds)
      ! open(111,file="Shat_s")
      !   do i=1,size(Shat,2),1
      !      write(111,'(1000(F8.4,2X))')Shat(:,i)
      !   enddo
      ! close(111)
      ! open(111,file="Qs_s")
      !   do i=1,size(qs,2),1
      !      write(111,'(1000(F8.4,2X))')qs(:,i)
      !   enddo
      ! close(111)
#else
   ...
   SUBROUTINE Rayleigh_quotient_ISO(veff_3D,nst,x,xhx)
      ...
      !LOCAL
#ifndef MPI
      REAL(DP) :: hx(rho_calc%OneDLength,nst)
#else
   ...
   SUBROUTINE cal_HX_ISO(veff,nst,V,HV)
      ...
      LOGICAL :: l1=.false.
#ifndef MPI
      REAL(DP) :: V_local(rho_calc%OneDLength)
      REAL(DP) :: HV_local(rho_calc%OneDLength)
#else
      ...
      DO Is=1,nst,1
         ...
         HV_local=HV(:,Is)
#ifndef MPI
         CALL ISO_rmatvec(veff,V_local,HV_local,rho_calc%OneDLength)
#else
   ...
   SUBROUTINE cheby_filtering_GRRiso(veff_3D,X_sphe,D)
     ...
     ! USE array_io
      ...
      !> filtering (a,b)
#ifdef MPI
      MPI_SUMTIME("chebyshev_filter_scaled_ISO",l1,chebyshev_filter_scaled_ISO(veff_3D,X_sphe,CheM,a,b,al))
#else
      CALL chebyshev_filter_scaled_ISO(veff_3D,X_sphe,CheM,a,b,al)
#endif
      !> GRR (Rayleigh-Ritz Step)
#ifdef MPI
      MPI_SUMTIME("GRayleigh_Ritz_ISO",l1,GRayleigh_Ritz_ISO(veff_3D,X_sphe,D))
#else
      CALL GRayleigh_Ritz_ISO(veff_3D,X_sphe,D)
#endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   ...
   SUBROUTINE Estupb_ISO(k,veff_3D,vec,b)
      ...
      !LOCAL
#ifndef MPI
      REAL(DP) :: v0(rho_calc%OneDLength),v(rho_calc%OneDLength),f(rho_calc%OneDLength)
      REAL(DP) :: alpha,beta,eval(k),mz
#else
      ...
      !
#ifndef MPI
      CALL ISO_rmatvec(veff_3D,v,f,rho_calc%OneDLength)
#else
   ...
   SUBROUTINE chebyshev_filter_scaled_ISO(veff_3D,X,m,a,b,al)
     ...
     USE m_time_evaluate, ONLY: memory_sum,memory_free
      ...
      !
#ifndef MPI
      REAL(DP),DIMENSION(rho_calc%OneDLength,sn)  :: HV,Y,Ynew
#else
   ...
   SUBROUTINE GRayleigh_Ritz_ISO(veff_3D,X,D)
...
#ifndef MPI
      REAL(DP),DIMENSION(sn,sn) :: S_hat,H_hat,Q
      REAL(DP) :: Xnew(rho_calc%OneDLength,sn)
#else
      ...
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#ifndef MPI
      call memory_sum('Rayleigh_ritz_local',real(size(S_hat),DP)*3*DP+size(Xnew)*DP)
#else
      ...
      !Calculate the project hamiltion
#ifndef MPI
      CALL Rayleigh_quotient_ISO(veff_3D,sn,X,H_hat)
#else
      ...
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
#ifndef MPI
      call memory_free('Rayleigh_ritz_local',real(size(S_hat),DP)*3*DP+size(Xnew)*DP)
#else
...
! #endif
   ...
   SUBROUTINE cheby_filter_RR_gamma(Ik,veff,X,D)
      ...
      al=MINVAL(D)
#ifndef MPI
      CALL Estupb_gamma(7,Ik,veff,X(:,sn),b)
#else
      !> find the core deal with the highest state
      aa:DO j=parallel%dims(1),1,-1 !> all core
      DO i=size(parallel%sub2sum,1),1,-1  !> all states per core
