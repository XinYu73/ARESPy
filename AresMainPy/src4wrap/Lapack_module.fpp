...
CONTAINS
  ...
  !#########################################################!
   SUBROUTINE OrthNorm(mat)
      ...
      COMPLEX(DP) :: tau(SIZE(mat,2))
      COMPLEX(DP) :: work(5000)!work(SIZE(mat,2))
      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      m=SIZE(mat,1)
      n=SIZE(mat,2)
      lda=MAX(1,m)
