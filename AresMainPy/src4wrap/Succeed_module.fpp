...
MODULE succeed
  ...
  REAL(DP),allocatable :: rho1(:,:),rho2(:,:),rho3(:,:)&
       ...
       ,rho_at(:,:),rhoi(:,:,:)&
       ,rho_at1(:,:),rhoi1(:,:,:)
  COMPLEX(DCP),allocatable :: psi1_c(:,:,:,:),psi2_c(:,:,:,:),psi3_c(:,:,:,:)
  !> grid info
  INTEGER(I4B) :: counter1=0,counter2=0,counter3=0
  REAL(DP) :: alpha,beta

  INTERFACE store_psi
     MODULE PROCEDURE store_psi_cmplx
     MODULE PROCEDURE store_psi_real
  END INTERFACE store_psi
  INTERFACE get_psi
     MODULE PROCEDURE get_psi_cmplx
     MODULE PROCEDURE get_psi_real
  END INTERFACE get_psi

CONTAINS
  SUBROUTINE init_succeed_rho_real(n_rho,n_r,n_s,nspin,dv)
  ...
  ENDSUBROUTINE init_succeed_rho_real

  SUBROUTINE init_succeed_rho_cmplx(n_rho,n_r,n_s,n_k,nspin,dv)
    USE m_time_evaluate, ONLY: memory_sum
    implicit none
    !> number of local rho grid
    INTEGER(I4B) :: n_rho
    ...
    r2=0.d0
    r3=0.d0
    allocate(psi1_c(n_rho,n_s,n_k,nspin))
    allocate(psi2_c(n_rho,n_s,n_k,nspin))
    allocate(psi3_c(n_rho,n_s,n_k,nspin))
    psi1_c=0.d0
    psi2_c=0.d0
    psi3_c=0.d0
    call memory_sum('succeed_rho',real(size(rho1),DP)*4*DP+size(r1)*3*DP+size(psi1)*3*DP)
  ENDSUBROUTINE init_succeed_rho_cmplx

  SUBROUTINE destroy_succeed()
    USE m_time_evaluate, ONLY: memory_free
    implicit none

    call memory_free('succeed_rho',real(size(rho1),DP)*4*DP+size(r1)*3*DP+size(psi1)*3*DP)
    if(allocated(rho1))deallocate(rho1,rho2,rho3)
    if(allocated(r1))deallocate(r1,r2,r3)
    if(allocated(psi1))deallocate(psi1,psi2,psi3)
    if(allocated(psi1_c))deallocate(psi1_c,psi2_c,psi3_c)
    if(allocated(rho_at))deallocate(rho_at)
    counter1=0;counter2=0;counter3=0

  ENDSUBROUTINE destroy_succeed

  SUBROUTINE store_rho(n_rho,Nspin,rho,dvol)
    implicit none
  ...
  SUBROUTINE store_psi_real(n_rho,n_s,nspin,psi)
    implicit none
    !> array dimension
  ...
  ENDSUBROUTINE store_psi_real

  SUBROUTINE store_psi_cmplx(n_rho,n_s,n_k,nspin,psi)
    implicit none
    !> array dimension
    INTEGER(I4B) :: n_rho,n_s,n_k,nspin
    !> psi
    COMPLEX(DCP) :: psi(n_rho,n_s,n_k,nspin)
  ...
  ENDSUBROUTINE store_psi_cmplx

  SUBROUTINE get_rho(nr,r_new,nrho,nspin,rho_new,dvol)
    USE math , ONLY: Norm
    implicit none
    ...
    rho_new=rho_new*ncharge/tele/dvol
  ...
  SUBROUTINE get_psi_real(nrho,n_s,nspin,psi_new)
    implicit none
  ...
  ENDSUBROUTINE get_psi_real

  SUBROUTINE get_psi_cmplx(nrho,n_s,n_k,nspin,psi_new)
    implicit none
    INTEGER(I4B),intent(IN)     :: nrho,n_s,n_k,nspin
    COMPLEX(DCP),intent(OUT)    :: psi_new(nrho,n_s,n_k,nspin)
    !> local
    ! REAL(DP) :: alpha,beta
    ! REAL(DP) :: a11,a22,a12,b1,b2
    ! REAL(DP) :: detA
    ...
    if(counter3>3)then
       dimension_psi = nrho*n_s*n_k*nspin
       CALL dcopy(dimension_psi, psi1_c,1,psi_new,1)
       CALL daxpy(dimension_psi, alpha,psi1_c,1,psi_new,1)
       CALL daxpy(dimension_psi, beta-alpha,psi2_c,1,psi_new,1)
       CALL daxpy(dimension_psi, -beta,psi3_c,1,psi_new,1)
    else
       psi_new=psi1_c
    endif
  ...
  SUBROUTINE cal_trans_phase(nr,nspin,r_new,n1,n2,n3,ng1,ng2,ng3,gvec,trans_phase)
    ...
    COMPLEX(DP)  :: eigr
    COMPLEX(DP)  :: rhoi_reci(ng1,ng2,ng3,nspin,nr),rho_at_reci(ng1,ng2,ng3,nspin)
    COMPLEX(DP)  :: rhoi_over_rhoat(ng1,ng2,ng3)
  ...
  SUBROUTINE get_new_rho_psi(nr,r_new,nrho,n1,n2,n3,nspin,rho_new,n_s,psi_new,gvec)
    ...
    COMPLEX(DP),allocatable :: trans_phase(:,:,:,:),rho1_reci(:,:,:)
