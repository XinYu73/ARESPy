...
#ifndef MPI
      Evxc=ddot(size(rhoS),vxc,1,rhoS,1)*dvol
#else
      ix=mod(mod(parallel%mygrid_range(1)-1,n1*n2),n1)+1
      iy=mod((parallel%mygrid_range(1)-1)/n1,n2)+1
      iz=1
...
! #endif
   ...
   SUBROUTINE Ehartree(rho,vhart,Ehart)
      ...
      !>>>>>>>>>>>>>>>>>>>>> Main Body >>>>>>>>>>>>>>>>>>>>>>>>
#ifndef MPI
      Ehart=0.5d0 * ddot(size(vhart),vhart,1,rho,1)*dvol
#else
      ix=mod(mod(parallel%mygrid_range(1)-1,n1*n2),n1)+1
      iy=mod((parallel%mygrid_range(1)-1)/n1,n2)+1
      iz=1
   ...
   SUBROUTINE ISO_totalenergy(rhoS,eval,fmenergy,ets,Llast)
     ...
     !------------------total energy calculation------------------
      ...
      IF(Llast)THEN
        ...
        !ion-e local part
#ifndef MPI
        CALL ISO_E_ie(rho,grid%vlpp,Eie)
#else
...
#ifndef MPI
     Eie=ddot(size(rho),rho,1,Vionlpp,1)*dvol
#else
