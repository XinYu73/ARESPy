...
#else
       WRITE(6,*) 'Initialize density : by pseudopotential files'
#endif
       CALL inichrg()
     !=================================================
     ELSE
...
#else
        WRITE(6,*) 'Initialize density : by uniform density'
#endif
        !uniform density
        IF(NSPIN==1)THEN
...
#else
     USE grid_module , ONLY : n1,n2,n3,n,grid,dvol
#endif
     IMPLICIT NONE
     !LOCAL
     REAL(DP) :: rho_t(n)
...
! #endif
     ...
     ELSE
        ...
        DO ix=1,n1
           I=0
           grid%rhoS(ix,iy,iz,1)=rho_t(I)*0.48d0
           grid%rhoS(ix,iy,iz,2)=rho_t(I)*0.52d0
        ENDDO
        ENDDO
        ENDDO
#endif
     ...
     iy=mod((parallel%mygrid_range(1)-1)/n1,n2)+1
     allocate(rho_temp(global_n1,global_n2,global_n3))
     ! call MPI_ALLGATHERV(grid%rhoS(ix,iy,1,1),parallel%mygrid_range(3),&
     call MPI_ALLGATHERV(rho_t(:),parallel%mygrid_range(3),&
          & MPI_REAL8,rho_temp,parallel%recvcounts,parallel%displs&
          &, MPI_REAL8, parallel%commx,mpinfo)
     open(1111+parallel%myid,file="init_density"//filename(10:11))
     write(1111+parallel%myid,*)shape(rho_temp)
     write(1111+parallel%myid,*)rho_temp
...
!STOP
  ...
  SUBROUTINE init_ISO_density()
...
#ifndef MPI
    IF(IDinit==2)THEN
      WRITE(6,*) 'Initialize ISO density : by MO method'
      CALL init_MO_Density_iso()
    !=====================================================================
    ELSEIF(LRadRho.and.IDinit==0)THEN
    !IF(LRadRho.and.IDinit==0)THEN
       WRITE(6,*) 'Initialize ISO density : by pseudopotential files'
#else
    IF(LRadRho.and.IDinit==0)THEN
    ...
    ELSE
      !##uniform density
#ifndef MPI
       WRITE(6,*) 'Initialize ISO density : by uniform files'
#else
  ...
  SUBROUTINE inichrg_iso()
     ...
     REAL(DP)    :: grid_rho(n)
#else
     REAL(DP) :: rho_atom(n,nspin,natom),rho_atom_s(rho_calc%OneDLength,natom)
     INTEGER(I4B) :: In
     REAL(DP) :: a = 3
#endif
     !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
#else
     call memory_sum('init_density_local',real(size(rho_atom)+size(rho_atom_s),DP)*DP)
#endif
     rho_t=0.d0
#ifndef MPI
     rho_atom=0.d0
...
#ifndef MPI
#else
     print*,'rmax',psp(1)%rmax
      if(parallel%myid==1)then
      open(111,file="rhot")
      write(111,*)psp(1)%denr
      ! write(111,*)rho_calc%x
      ! write(111,*)rho_calc%y
      ! write(111,*)rho_calc%z
      ! write(111,*)gap
      ! write(111,*)struct%poscar
      close(111)
      endif
#endif
#endif
     ...
     !
     rho_t(:)=rho_t(:)*ncharge/tele
     rho_atom_s=rho_atom_s*ncharge/tele
     !> decide the initial density
     !> assignment to grid
     grid%rhoS=0.d0
     IF(NSPIN==1)THEN
     ...
     ELSE
       DO i=1,rho_calc%OneDLength
         ...
         grid%rhoS(rho_calc%x(i),rho_calc%y(i),rho_calc%z(i),2)= &
             & rho_t(i)*0.52d0
          In=rho_calc%x(i)+n1*(rho_calc%y(i)-1)+n1*n2*(rho_calc%z(i)-1)
          rho_atom(In,1,:)=rho_atom_s(i,:)*0.48
          rho_atom(In,1,:)=rho_atom_s(i,:)*0.52
       ENDDO
     ENDIF
       ! CALL store_rho_at_fft_trans(n,Nspin,natom,grid%rhoS,rho_atom)
       ! print *,'store rho rhoi'
#else
     !> assignment to grid
     ! grid%rhoS=0.d0
     ! IF(NSPIN==1)THEN
...
#else
     call memory_free('init_density_local',(real(size(rho_atom),DP)+size(rho_atom_s))*DP)
#endif
     !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
