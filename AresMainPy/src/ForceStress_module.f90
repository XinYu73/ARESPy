...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   SUBROUTINE cal_force_c(rhoS,Uik,force)
      ...
      force(:,:)=force(:,:) + tmpf(:,:)
      ! if(parallel%isroot)print*,'nloc',tmpf
      !===================================
   ...
   !-----------------------DIVIDER-LINE--------------------------
   SUBROUTINE cal_force_gamma(rhoS,Uik,force)
      USE struct_module, ONLY: natom,struct,lat_mat
      USE ewald , ONLY : ewald_forces,ISO_ewald_forces
      USE pspot_module ,ONLY: psp
      USE parameters   ,ONLY: LBvK
...
#ifdef MPI
      COMPLEX(DP) :: rhog(ng1,ng2,parallel%local_z) , sfactor
#else
      COMPLEX(DP) :: rhog(ng1,ng2,ng3) , sfactor
#endif
...
!------------------------nonlocal force--------------------
   SUBROUTINE nonlforce(Uik,nlforce)
      ...
      IMPLICIT NONE
      !IN/OUT
      COMPLEX(DP),INTENT(IN) :: Uik(:,:,:,:) !wave_function
      ...
      COMPLEX(DP) :: dGnlm(3),Gnlm,tmp0(3)
#ifdef MPI
      COMPLEX(DP) :: tmp0_global(3)
#endif
      !psi_ik=e^ikr U_ik
      COMPLEX(DP)  :: psi_ik(max_nlnpts,Nstates,nk,NSPIN) &
                &,    eikr
         ...
         DO Ia=struct%eleid(Ity),struct%eleid(Ity+1)-1
            ...
            DO Ipj=1,psp(Ity)%nproj
               ...
               DO Ip=1,nlpot(Ia)%npts
                  ...
                  ! else
                  ! pos=0
                  ! DO WHILE ( psp(Ity)%r_real(pos+2).lt.xyzr(4,Ip) .and. (pos.le.psp(Ity)%numps-2))
                  !    pos=pos+1
                  ! ENDDO
                  ! dbeta=polynom(1,5,psp(Ity)%r_real(pos-2:pos+2),psp(Ity)%beta_r(pos-2:pos+2,Ipj),c,xyzr(4,Ip))
                  ! endif
                  !dylm/dr_R
            ...
            DO Ipj=1,psp(Ity)%nproj
               ...
               DO Ii=1,Nstates
                  ...
                  ENDDO
#ifndef MPI
                  tmp0(:)=tmp0(:)+wke(Ii,Ik,Is)*Gnlm*dGnlm(:)
#else
...
! #endif
   ...
   !-----------------------DIVIDER-LINE--------------------------
   SUBROUTINE nonlforce_gamma(Uik,nlforce)
      USE math , ONLY : SimpleInterp,r_dYlm
      USE MathSplines, ONLY: polynom
      USE nlpot_module , ONLY : nlpot,max_nlnpts
      USE struct_module ,ONLY : naty,struct,recip_lat
         ...
         DO Ia=struct%eleid(Ity),struct%eleid(Ity+1)-1
            ...
            DO Ipj=1,psp(Ity)%nproj
               ...
               DO Ii=1,Nstates
                  ...
                  tmp0(:)=tmp0(:)+wke(Ii,Ik,Is)*Gnlm*dGnlm(:)
#else
                  CALL MPI_ALLREDUCE(Gnlm,Gnlm_global,1,mpi_real8,mpi_sum,parallel%commx,mpinfo)
                  tmp0(:)=tmp0(:)+wke(parallel%sub2sum(Ii,parallel%ranky+1),Ik,Is)*Gnlm_global*dGnlm(:)
#endif
               ENDDO
               ENDDO
...
! #endif
     ...
     DO Ia=struct%eleid(Ity),struct%eleid(Ity+1)-1
        ...
        DO Ip=1,nlpot(Ia)%npts
           ...
           Ik=1
#ifndef MPI
           psi_ik(Ip,:,Ik,:)=Uik(Id,:,Ik,:)!eikr*Uik(Id,:,Ik,:)
#else
...
! #endif
        ...
        !4.calculate the nonlocal force (dvol for scale)
#ifndef MPI
        nlforce(:,Ia)=tmpf(:)*dvol*2.d0*2.d0
#else
...
#else
     call memory_sum('nolocal_force',(real(size(xyzr),DP)+size(psi_ik))*DP)
#endif
     nlforce(:,:)=0.d0
     !Cycle all species
     DO Ity=1,naty
...
#else
     call memory_free('nolocal_force',(real(size(xyzr),DP)+size(psi_ik))*DP)
#endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
#ifdef MPI
   COMPLEX(DP) :: rhoq(ng1,ng2,parallel%local_z)
#else
   COMPLEX(DP) :: rhoq(ng1,ng2,ng3)
#endif
   INTEGER(I4B) :: Is,i,j
#ifdef MPI
   REAL(DP) :: rho_FFT(global_n1,global_n2,parallel%local_z)
...
ENDSUBROUTINE cal_stress
   !-----------------------DIVIDER-LINE--------------------------
SUBROUTINE cal_stress_gamma(rhoS,Uik,stress)
   USE struct_module, ONLY : struct,lat_mat
   USE ewald , ONLY : ewald_stress
   USE grid_module , ONLY: n1,n2,n3,ng1,ng2,ng3
   USE parameters , ONLY : Nspin
   ...
   !> nonlocal stress
   if(max_nproj/=0)then
      CALL ion_nl_stress_gamma(Uik,tmps)
      stress(:,:)=stress(:,:) + tmps(:,:)
   endif
   ! if(parallel%isroot)print*,'nonlocal stress'
   ! if(parallel%isroot)print*,tmps
   ...
   !> kinetic stress
   CALL Kin_stress_gamma(Uik,tmps)
   stress(:,:)=stress(:,:) + tmps(:,:)
   ! if(parallel%isroot)print*,'Kinetic stress'
   ! if(parallel%isroot)print*,tmps
...
#endif
   !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 ENDSUBROUTINE cal_stress_gamma
!--------------------LDA XC stress-------------------------
...
!---------------------kinetic stress-----------------------
   SUBROUTINE Kin_stress(Uik,Kinstress)
      ...
      !LOCAL
      !COMPLEX(DP)  :: eikr(n)
      COMPLEX(DP)  :: dUik(3,n1,n2,n3),tUik(n1,n2,n3)
   ...
   SUBROUTINE Kin_stress_gamma(Uik,Kinstress)
      USE parameters , ONLY : Nspin,Nstates,norder=>finite_order
      USE grid_module , ONLY : n1,n2,n3,n,nk,grid,KPT
      USE struct_module , ONLY : volume
      ...
      !LOCAL
      !COMPLEX(DP)  :: eikr(n)
      REAL(DP)  :: dUik(3,n1,n2,n3),tUik(n1,n2,n3)
      INTEGER(I4B) :: Is,Ik,Ii,Ix,Iy,Iz,I
#ifdef MPI
      INTEGER(I4B) :: in,j,k
      REAL(DP) :: Kinstress_local(3,3)
      ...
      DO Is=1,Nspin
         ...
         DO Ik=1,nk
            ...
            DO Ii=1,Nstates
...
#endif
               !calculate the gridient of psi
               CALL nabla_gamma(tUik,norder,dUik)
               !store something
               sum_u2 = SUM(tUik*tUik)
               !udui
               sum_udu(1)=SUM(tUik(:,:,:)*dUik(1,:,:,:))
               sum_udu(2)=SUM(tUik(:,:,:)*dUik(2,:,:,:))
               sum_udu(3)=SUM(tUik(:,:,:)*dUik(3,:,:,:))
               !dudu
               sum_dudu(1)=SUM(dUik(1,:,:,:)*dUik(1,:,:,:))
               sum_dudu(2)=SUM(dUik(1,:,:,:)*dUik(2,:,:,:))
               sum_dudu(3)=SUM(dUik(1,:,:,:)*dUik(3,:,:,:))
               sum_dudu(4)=SUM(dUik(2,:,:,:)*dUik(2,:,:,:))
               sum_dudu(5)=SUM(dUik(2,:,:,:)*dUik(3,:,:,:))
               sum_dudu(6)=SUM(dUik(3,:,:,:)*dUik(3,:,:,:))
               !calculate the temp stress
               !1:11
               tmp0(1)=  sum_u2*kk(1) + sum_dudu(1)      &
                   &   + IMAG*kvec(1)*sum_udu(1)  &
               ...
               !2:12
               tmp0(2)=  sum_u2*kk(2) + sum_dudu(2)      &
                   &   + IMAG*kvec(2)*sum_udu(1)  &
                   &   - IMAG*kvec(1)*sum_udu(2)
               !3:13
               tmp0(3)=  sum_u2*kk(3) + sum_dudu(3)      &
                   &   + IMAG*kvec(3)*sum_udu(1)  &
                   &   - IMAG*kvec(1)*sum_udu(3)
               !4:22
               tmp0(4)=  sum_u2*kk(4) + sum_dudu(4)      &
                   &   + IMAG*kvec(2)*sum_udu(2)  &
...
#endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   ENDSUBROUTINE Kin_stress_gamma
!-----------------------nl stress--------------------------
   SUBROUTINE ion_nl_stress(Uik,nlstress)
      ...
      COMPLEX(DP)  :: eikr,tmps(6),tmp0(6),tG0,tGs
#ifdef MPI
      COMPLEX(DP)  :: tGs_global
#endif
      COMPLEX(DP)  :: dGnlm(6),Gnlm,coe(6),tylm(6)   !
#ifdef MPI
      COMPLEX(DP)  :: dGnlm_local(6),Gnlm_local,tmps_global(6)
...
#else
                  tmp0(:)=tmp0(:) + wke(Ii,Ik,Is)*CONJG(Gnlm)*dGnlm(:)
                  tG0 = tG0 + wke(Ii,Ik,Is)*CONJG(Gnlm)*Gnlm
#endif
               ENDDO
               ENDDO
               ENDDO
      ...
      ENDDO
#ifdef MPI
      CALL MPI_ALLREDUCE(tmps,tmps_global,6,mpi_complex16,mpi_sum,parallel%comm,mpinfo)
      tmps=tmps_global
      ! !> tGs need to be reduced in commy while tmps have reduced in comm
      CALL MPI_ALLREDUCE(tGs,tGs_global,1,mpi_complex16,mpi_sum,parallel%commy,mpinfo)
      tGs=tGs_global
#endif
      !out the stress
...
#else
      nlstress(:,:)=nlstress(:,:)/REAL(n,DP)
#endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   ENDSUBROUTINE ion_nl_stress
   SUBROUTINE ion_nl_stress_gamma(Uik,nlstress)
      USE parameters , ONLY : Nstates,Nspin,PP_identifer
      USE nlpot_module , ONLY : max_nlnpts,nlpot
      USE pspot_module , ONLY : max_nproj,psp
      USE struct_module , ONLY : naty,struct
      ...
      !index
      INTEGER(I4B) :: Ity,Ia,Ip,Id,Is,Ik,Ii,Ipj,l,m
      REAL(DP)  :: eikr,tmps(6),tmp0(6),tG0,tGs
#ifdef MPI
      REAL(DP)  :: tGs_global
#endif
      REAL(DP)  :: dGnlm(6),Gnlm,coe(6),tylm(6)   !
#ifdef MPI
      REAL(DP)  :: dGnlm_local(6),Gnlm_local,tmps_global(6)
      REAL(DP)     :: nlstress_local(3,3)
#endif
      !> interplote
      REAL(DP) :: c(3)
...
#ifdef MPI
                  Gnlm_local=Gnlm
                  CALL MPI_ALLREDUCE(Gnlm_local,Gnlm,1,mpi_real8,mpi_sum,parallel%commx,mpinfo)
                  ! dGnlm_local(:)=dGnlm(:)
                  ! CALL MPI_ALLREDUCE(dGnlm_local,dGnlm,6,mpi_complex16,mpi_sum,parallel%commx,mpinfo)
                  tmp0(:)=tmp0(:) + wke(parallel%sub2sum(Ii,parallel%ranky+1),Ik,Is)*Gnlm*dGnlm(:)
                  tG0 = tG0 + wke(parallel%sub2sum(Ii,parallel%ranky+1),Ik,Is)*Gnlm*Gnlm
#else
                  tmp0(:)=tmp0(:) + wke(Ii,Ik,Is)*Gnlm*dGnlm(:)
                  tG0 = tG0 + wke(Ii,Ik,Is)*Gnlm*Gnlm
#endif
               ENDDO
               ENDDO
               ENDDO
      ...
      ENDDO
#ifdef MPI
      CALL MPI_ALLREDUCE(tmps,tmps_global,6,mpi_real8,mpi_sum,parallel%comm,mpinfo)
      tmps=tmps_global
      ! !> tGs need to be reduced in commy while tmps have reduced in comm
      CALL MPI_ALLREDUCE(tGs,tGs_global,1,mpi_real8,mpi_sum,parallel%commy,mpinfo)
      tGs=tGs_global
#endif
      !out the stress
      nlstress(1,1)=REAL( tGs+2*tmps(1), DP )
...
#endif
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   ENDSUBROUTINE ion_nl_stress_gamma
!----------------------local stress------------------------
   function IonEle_stress(rhoRecip,energy)!{{{
      ...
      complex(DP),dimension(:,:,:),intent(in) :: rhoRecip
...
#else
      IonEle_stress= IonEle_stress + IE_stress
#endif
      !$OMP END CRITICAL
      !$OMP END PARALLEL
     !print *,"I0"
...
!---------------------cal_force_stress---------------------
   SUBROUTINE cal_force_stress()
      ...
      USE parameters , ONLY : LBvK,Idiag,NPRR,Lpbc,CalForce&
           & ,hartree_method,LGamma
      USE begin_module , ONLY : Initial_grid,Initial_density,init_ISO_density
      USE grid_module , ONLY : grid,eigen,eigen_r,rho_calc,n,dvol
      USE potential_module , ONLY : vlpp
      USE nlpot_module , ONLY : initialize_nlpot
#ifndef MPI
      USE scf_module , ONLY : ArpackSCF,CheFSI &
                          &,  ArpackSCF_r,BvK_CheFSI,pRR_CheFSI,&
                          & ISO_ArpackSCF_r, ISO_CheFSI
#else
      USE scf_module , ONLY :  ISO_CheFSI, CheFSI,CheFSI_Gamma
#endif
      ...
      USE parameters , ONLY : Nspin,Nstates
#ifdef MPI
      USE grid_module , ONLY : gap,n1,n2,n3,global_n1,global_n2,global_n3,nk
      USE array_io    , ONLY : output
      USE succeed     , ONLY : store_rho_at,store_rho,store_r,store_psi,Llastrho
#else
      USE succeed     , ONLY : store_rho_at,store_rho,store_r,store_psi,Llastrho
      !store_rho_fft_trans,store_r_fft_trans,store_psi_fft_trans,Llastrho
#endif
      USE m_time_evaluate, ONLY: memory_sum
      IMPLICIT NONE
      !LOCAL
      !COMPLEX(DP),ALLOCATABLE  :: rho_grid(:,:,:,:)
#ifdef MPI
      ...
      ELSE
         ...
         CALL store_rho_at(rho_calc%OneDLength,Nspin,rho_calc%OneDSphere)
#else
      ...
      !Self-consistent
#ifndef MPI
      IF(LBvK.AND.Lpbc)THEN
         IF(Idiag==0)THEN
            !by Arpack solver
            CALL ArpackSCF_r(grid%rhoS,eigen_r%wvf,eigen_r%val)
         ELSEIF(NPRR>0)THEN
            CALL pRR_CheFSI(grid%rhoS,eigen_r%wvf)
         ELSE
            CALL BvK_CheFSI(grid%rhoS,eigen_r%wvf,eigen_r%val)
         ENDIF
      ELSE
         IF(Idiag==0.AND.Lpbc)THEN
            !by Arpack solver
            CALL ArpackSCF(grid%rhoS,eigen%wvf,eigen%val)
         ELSEIF(Idiag==0.AND..NOT.Lpbc)THEN
            CALL ISO_ArpackSCF_r(grid%rhoS,rho_calc%OneDwvf,rho_calc%OneDval)
         ELSEIF(Idiag==1.AND..NOT.Lpbc)THEN
           print*,"!===========temporary============!"
           print*,"sphere size",size(rho_calc%OneDwvf,1)
           print*,"gap",gap
           print*,"!===========temporary============!"
           CALL ISO_CheFSI(grid%rhoS,rho_calc%OneDwvf,rho_calc%OneDval)
            ! print *,"OneDval", rho_calc%OneDval
         ELSE
            !by Chebyshev-filter
      ...
      if(Lpbc)then
         if(LGamma)then
            CALL CheFSI_Gamma(grid%rhoS,eigen_r%wvf,eigen_r%val)
         else
            CALL CheFSI(grid%rhoS,eigen%wvf,eigen%val)
         endif
      else
...
#else
      if(parallel%isroot)then
      open(111,file="rhoS_f")
      write(111,*)rho_calc%OneDSphere
      close(111)
      endif
#endif
#endif
      ...
      if(.not.Lpbc)then
         ...
         CALL store_rho(parallel%mygrid_range(3),Nspin,rho_calc%OneDSphere,dvol)
         ...
         !    eigen_r%wvf(In,:,1,:)=rho_calc%OneDwvf(i,:,1,:)
         ! ENDDO
         CALL store_r(natom,struct%poscar)
         CALL store_rho(size(grid%rhoS),Nspin,grid%rhoS,dvol)
         CALL store_psi(rho_calc%OneDLength,Nstates,Nspin,rho_calc%OneDwvf)
         ! CALL store_psi_fft_trans(n,Nstates,Nspin,eigen_r%wvf)
         ! CALL store_r_fft_trans(natom,struct%poscar)
         ! CALL store_rho_fft_trans(n,Nspin,grid%rhoS)
#endif
      else
         CALL store_r(natom,struct%poscar)
         CALL store_rho(parallel%mygrid_range(3),Nspin,grid%rhoS,dvol)
         if(LGamma)then
            CALL store_psi(parallel%mygrid_range(3),Nstates,Nspin,eigen_r%wvf(:,:,1,:))
         else
            CALL store_psi(parallel%mygrid_range(3),Nstates,nk,Nspin,eigen%wvf)
         endif
      endif

      !> cal force and stress
10011 IF(.NOT.LBvK.AND.Lpbc.AND.CalForce)THEN  !##periodic
         if(Lgamma)then
            !calculate the forces
            CALL cal_force_gamma(grid%rhoS,eigen_r%wvf,struct%forces)
            !calculate the stress
            ! eigen%wvf=(0.001,0.001)
            CALL cal_stress_gamma(grid%rhoS,eigen_r%wvf,struct%stress)
         else
            !calculate the forces
