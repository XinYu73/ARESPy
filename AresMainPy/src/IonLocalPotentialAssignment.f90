...
CONTAINS
  ...
  SUBROUTINE CalVlpp()
    ...
    INTEGER(I4B)         :: i,j
#ifndef MPI
    REAL(DP)             :: temp(n1,n2,n3),Vsphere(rho_calc%OneDLength,1) &
         &, Vtemp(rho_calc%OneDLength,1)
#else
  ...
  SUBROUTINE IonPotentialAssignment(Ity,zion,poscar,temp)
    ...
    IMPLICIT NONE
#ifndef MPI
    REAL(DP),INTENT(OUT)       :: temp(n1,n2,n3)
#else
    ...
    DO k=1,n3,1
       DO j=1,n2,1
          DO i=1,n1,1
             ...
             DirPos=(/(i-1)/real(n1,DP),(j-1)/real(n1,DP),(k-1)/real(n1,DP)/)
             CALL dir2car_single(DirPos,CarPos,lat_mat)
             CarPos=CarPos+gap
             rr=sqrt((CarPos(1)-poscar(1))**2+(CarPos(2)-poscar(2))**2+(CarPos(3)-poscar(3))**2)
#else
...
#ifndef MPI
                if(m.le.max_m)then
                   temp(i,j,k)=polynom(0,3,psp(Ity)%r_real(m-1:m+1),psp(Ity)%V_loc(m-1:m+1),c,rr)
                else
                   temp(i,j,k)=-psp(Ity)%Zion/rr
                endif
#else
                if(m.le.max_m)then
       ...
       ENDDO     !> k=1,n3,1
#else
    enddo     !> i=1,parallel%mygrid_range(3),1
#endif
    !=======================================================================
    ! close(998)
    ! close(999)
...
#else
                   CALL set_Vloc_dg(Ity,rr_3d,temp(i,j,k))
#endif
                else
                   m=1
