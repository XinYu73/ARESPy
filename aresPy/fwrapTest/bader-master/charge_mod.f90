...
MODULE charge_mod
  ...
  PUBLIC :: charge_obj_initialise, charge_obj_finalise, get_weights
...
!-----------------------------------------------------------------------------------!
    ...
    SUBROUTINE charge_obj_initialise(chg, npts)

      TYPE(charge_obj), INTENT(INOUT) :: chg
      INTEGER, INTENT(IN) :: npts(3)

      CALL charge_obj_finalise(chg)
      chg%npts = npts
      ALLOCATE(chg%rho(npts(1), npts(2), npts(3)))

    END SUBROUTINE charge_obj_initialise

    SUBROUTINE charge_obj_finalise(chg)
      TYPE(charge_obj), INTENT(INOUT) :: chg
      INTEGER n1, n2, n3

      if (ALLOCATED(chg%weight)) then
         DO n1=1,chg%npts(1)
            DO n2=1,chg%npts(2)
               DO n3=1,chg%npts(3)
                  DEALLOCATE (chg%weight(n1,n2,n3)%w)
               END DO
            END DO
         END DO
         DEALLOCATE(chg%weight)
      end if
      IF (ALLOCATED(chg%rho)) DEALLOCATE(chg%rho)
      
    END SUBROUTINE charge_obj_finalise

    ! provide access to weights array - multi-dimensional array of derived types are not wrapped
    ! (James Kermode)
    
    SUBROUTINE get_weights(chg, w, status)
      TYPE(charge_obj), INTENT(IN) :: chg
      REAL(q2), DIMENSION(:,:,:,:), INTENT(INOUT) :: w
      ...
      if (.not. allocated(chg%weight)) then
         write (*,*) 'ERROR: chg%weight array not allocated'
         if (present(status)) status = 0
         return         
      end if

      if (.not. allocated(chg%weight(1,1,1)%w)) then
         write (*,*) 'ERROR: chg%weight(1,1,1) array not allocated'
         if (present(status)) status = 0         
         return         
      end if
      
      if (size(w,1) < chg%npts(1) .or. &
          size(w,2) < chg%npts(2) .or. &
          size(w,3) < chg%npts(3) .or. &
          size(w,4) < size(chg%weight(1,1,1)%w)) then
         write (*,*) 'ERROR: w array passed into get_weights() too small'
         write (*,*) 'should be at least', chg%npts, size(chg%weight(1,1,1)%w)
         if (present(status)) status = 0
         return
      end if
      ...
      do i1=1,chg%npts(1)
         do i2=1,chg%npts(2)
            do i3=1,chg%npts(3)
               w(i1, i2, i3, 1:size(chg%weight(i1,i2,i3)%w)) = chg%weight(i1,i2,i3)%w(:)
            end do
         end do
      end do
