...
CONTAINS
   ...
   SUBROUTINE real_PseudoSubspace(nps,nev,initX)
      ...
      ENDDO
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   ENDSUBROUTINE real_PseudoSubspace
   !----------------------------------------------------------
...
#else
         CALL GeneralizeEigen_real(nev,Hhat,Shat,Qs,D)
#endif
      ENDIF
      !-------------------
      !rotation
...
#else
      CALL matmat_real(X,Qs,'N','N',Xnew)
#endif
      !eigen-value
      X(:,:)=Xnew(:,:)
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  SUBROUTINE real_first_filter(nps,nst,veff,X,eval)
...
#else
            CALL GeneralizeEigen_real(nst,Hhat,Shat,Qs,eval)
#endif
         ENDIF
#ifdef MPI
   ...
   SUBROUTINE chebyshev_filter_real(nps,nst,veff,X,m,a,b)
      ...
      X=Ynew
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   ENDSUBROUTINE chebyshev_filter_real
...
#else
      CALL GeneralizeEigen_real(nev,H_hat,S_hat,Q,D)
#endif
...
!STOP
   ...
   SUBROUTINE Rayleigh_Ritz_real(nps,sn,veff,X,D)
      ...
      X=Xnew
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   ENDSUBROUTINE Rayleigh_Ritz_real
   ...
   SUBROUTINE cheby_filtering_GRRr(nps,nev,veff,X,D)
      ...
      !up boundary
#ifndef MPI
      CALL Estupb_real(nps,7,veff,X(:,nev),b)
#else
