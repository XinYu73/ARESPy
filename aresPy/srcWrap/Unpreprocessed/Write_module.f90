...
!---------------------------- DIVIDER LINE -----------------------------
#ifdef Pencil
SUBROUTINE Write3DPencil(filename, rho)!{{{
   !
   use struct_info, only : n_offset
   CHARACTER(LEN=*)                       :: filename
   REAL(DP), dimension(:,:,:) :: rho
   INTEGER(I4B)                  :: nxyz
   integer(kind=MPI_OFFSET_KIND) :: offset
   ...
   INTEGER(I4B)                  :: lsize(3)
   INTEGER(I4B)                  :: start(3)
   INTEGER(I4B)                  :: type_domain
   !-----------------------------------------------------------------------
   lsize = shape(rho)
   nxyz = lsize(1)*lsize(2)*lsize(3)
   gsize(1) = lsize(1)
   gsize(2) = lsize(2) * parallel%dims(1)
   gsize(3) = lsize(3) * parallel%dims(2)
   call MPI_FILE_OPEN(parallel%comm,trim(filename),MPI_MODE_WRONLY + MPI_MODE_CREATE, &
      MPI_INFO_NULL, funit,mpinfo)
   if (parallel%isroot) then
      offset=0
      call MPI_FILE_WRITE_AT(funit,offset,gsize, 3, MPI_INTEGER4, smpi_status, mpinfo) 
      print *, 'gsize',gsize
   endif
   start(:) = n_offset(:)
   call MPI_Type_create_subarray(3, gsize, lsize, start, MPI_ORDER_FORTRAN, MPI_REAL8, type_domain, mpierr)
   call MPI_Type_commit(type_domain, mpinfo)
   head = 3 * I4B
   offset = head
   call MPI_FILE_SET_VIEW(funit, offset, MPI_REAL8, type_domain, 'native',  MPI_INFO_NULL, mpinfo) 
   call MPI_FILE_WRITE(funit,rho, nxyz, MPI_REAL8, MPI_STATUS_IGNORE, mpinfo) 
   call MPI_FILE_CLOSE(funit,mpinfo) 
   !-----------------------------------------------------------------------
END SUBROUTINE!}}}
...
SUBROUTINE WriteDensity(filename, rho)!{{{
   ...
   REAL(DP), dimension(:,:,:) :: rho
#ifdef Pencil
   call Write3DPencil(filename,rho)
#else
